...
RSpec.describe PreservedObject, type: :model do
  ...
  context 'validation' do
    it 'is valid with required attributes' do
      expect(PreservedObject.create(required_attributes)).to be_valid
    end
    ...
    it 'enforces unique constraint on druid (db level)' do
      PreservedObject.create(required_attributes)
      dup_po = PreservedObject.new(druid: 'ab123cd4567', current_version: 2, preservation_policy: preservation_policy)
  ...
  describe '.normalize_druid_name' do
    context 'druid with prefix created ex. (druid:xx000xx0000)' do
      let(:po_with_prefix1) { create(:preserved_object, druid: 'druid:bb123cd4567') }
      let(:po_with_prefix2) { create(:preserved_object, druid: 'druid:ef891gh5300') }
      ...
      context 'matching record already exists' do
        before do
          create(:preserved_copy, preserved_object: create(:preserved_object, druid: 'bb123cd4567'), endpoint: Endpoint.first)
        end
      ...
      context 'rolls back transaction' do
        before do
          pc = create(:preserved_copy, preserved_object: create(:preserved_object, druid: 'bb123cd4567'), endpoint: Endpoint.first)
          ...
          allow(PreservedObject).to receive(:where).with(druid: ['bb123cd4567']).and_return([pc.preserved_object])
          expect(po_to_delete.first).to receive(:destroy).and_raise(ActiveRecord::ConnectionTimeoutError)
        end

        it 'pres obj is not deleted if pres copy cannot be deleted' do
          expect { PreservedObject.normalize_druid_name }.to raise_error(ArgumentError)
